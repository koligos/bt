...
Next, let's formulate some conditions for our constant ${\bi P} $ matrix. We call a matrix ${\bi P} $ {\em compatible} with a graph $G$, if $p_{ij}=0$ for $j\not\in {\cal{N}}_i$ (i.e. $i$-th node is not in a set of neighbors of the $j$-th node.) Still considering an undirected graph, we can write:
$${\bi P} = {\bi P}^T. \eqmark$$
...
$\alpha \in (0, \Delta],$ consensus is asymptotically reached for all initial states \cite[Consensus_and_Cooperation_in_Networked].
...
\endinsert
\label[ex3_7]
\Example For this moment the last experiment in Figure \ref[conv_noise], that we want to present over topology from Figure \ref[Graph], is adding reasonably small Additive White Gaussian Noise (AWGN) to the updates. Noisy updates will be main subject of the next chapter.
\midinsert
\picw=14cm \cinspic pictures/conv_noise.pdf
\clabel[conv_noise]{Example of average consensus algorithm with noisy updates.}
\caption/f Run of average consensus algorithm on graph from Figure \ref[Graph] with noisy updates. We can see, that adding noise to the updates in run of simple version of algorithm causes the algorithm to lose convergence property.
\endinsert
...
Note, the expression ${1\over N}{\bi 1}^T {\bi w}(t)$ is nothing but a sequence of random variables, which implies that $a(t)$ has the following properties \cite[Xiao]: \label[meaee]$$ {\bbchar E}[ a(t) ]= a(0) \eqmark$$ and \label[rozupdsf]$${\bbchar E}\left[ a(t) - {\bbchar E}[a(t)] \right]^2=t. \eqmark$$ (Note, in Eq. \ref[rozupdsf] we used a fact the $ var[{\bi w }(t)]=1.$) We emphasize, that \ref[rozupdsf] means, the variance of a random variable $a(t)$ increases linearly with time. It's also interesting, that nor \ref[meaee] nor \ref[rozupdsf] do not depend on the structure of matrix $\bi P$ \cite[Xiao]. Let's revisit the Figure \ref[conv_noise]. The increasing variance problem may be seen in Figure
...
\label[bucky]
\midinsert
\picw=6cm \cinspic pictures/bucky.pdf
...
\caption/f { The values in nodes of graph in run of 100 000 iterations of the algorithm from Example 3.8. Precious result is, that using the decending step size the algorithm does not diverge. }
\endinsert

We can see in Figure \ref[conv_noise_values_gamma] that such a modification of algorithm works and the values in all nodes converge to one value, altough very slowly. Selecting the sequence $\gamma(t)$ does not have any general rules (or at least we didn't manage to find them) and there is a tradeoff between speed of covergence and to value up to which the MSE decreases. In next Figure \ref[conv_noise_MSE_act_gamma] we can see in loglog graph, how the Mean square error of values truly decreases. In similiar Figure \ref[conv_noise_MSE_ave_gamma] we demonstrare another important fact: The MSE of actual values in each decreases according to Figure \ref[conv_noise_MSE_act_gamma], but they do not come to the average of initial values. This shows Figure \ref[conv_noise_MSE_ave_gamma]. MSE with respect an initial average will stop decreasing at some point. In this sense we have to say, that values in all nodes converge asympototically to one value, but they {\em do not converge} to the average of {\em initial values.}
...
\caption/f {Decreasing MSE w.r.t. moving average in nodes of graph in run of the algorithm from Example \ref[ex_conv_noise_gamma]. Decrease of this value consequently slows down, because variance of the noise comes to be still more significant w.r.t the small differences in updates.}
\endinsert
...
\picw=14cm \cinspic pictures/ring_noise_conv32.pdf
\clabel[conv_ring_val] {Values convergence from Example 3.9.}
\caption/f {Values convergence from Example 3.9.}
\endinsert

\midinsert
\picw=14cm \cinspic pictures/ring_noise_conv_variance.pdf

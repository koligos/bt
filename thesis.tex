...
average consensus algorithm over the graph. It is an iterative algorithm,
that works with values assigned to
vertices of the graph and the goal is to
asymptotically obtain in all vertices an average value of the initial values, respecting the topology of the graph. In the text, we have used a
matrix representation of graph and to
design the parameters of the algorithm is
...
are not included, but we have provided references, where is possible to find them in the literature. In the final part, we've implemented a few simple examples of the described algorithm, motivated by radio communication.
}
